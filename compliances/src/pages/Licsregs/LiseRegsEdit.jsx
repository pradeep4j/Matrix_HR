...
import {stateGets,usersGet,branchGet,companyGet,nameRateUpdate,documentCollectionUpdate,appDetailsDispatchUpdate,expenseDispatchUpdate,licenseInfoDispatchUpdate,invoiceInfoDispatchUpdate,companyInfoDispatchUpdate,regsGets,lisregsSaveandApprove,liseRegGettingById,companyTableGet,lisregsRejects} from "../../store/actions/otherActions";
import { useDispatch,useSelector } from 'react-redux';
import LisRegsTables from './LisRegsTables';
import { Modal, Button } from 'antd';
import Swal from 'sweetalert2';
const LiseRegsEdit = (props) => {
    const { editId } = props;
    ...
    const { expenseDetailsUpdateInfo } = updateExpense;
    console.log(expenseDetailsUpdateInfo) 
    const updateLicense = useSelector(state => state.updateLicense)
    const { licenseDetailsUpdateInfo } = updateLicense;
    console.log(licenseDetailsUpdateInfo) 
    const updatecreateInvoice = useSelector(state => state.updatecreateInvoice)
    const { invoiceDetailsUpdateInfo } = updatecreateInvoice;
    console.log(invoiceDetailsUpdateInfo) 
    const updateCompanyInfo = useSelector(state => state.updateCompanyInfo)
    const { companyInfoDetailsUpdateInfo } = updateCompanyInfo;
    console.log(companyInfoDetailsUpdateInfo) 

    const namerateU = useSelector(state => state.namerateU)
    const { namerateUpdateInfo } = namerateU;
    console.log(namerateUpdateInfo) 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmitModal = (e) => {
        ...
        dispatch(lisregsRejects(postBody));
    ...
    const handleSubmit = async (e) => {
        ...
        // alert(regsGetByIdInfo._id)
        dispatch(nameRateUpdate(formData,regsGetByIdInfo._id))
    ...
    const handleSubmitDocumentCollection = async (e) => {
        ...
        dispatch(documentCollectionUpdate(formData,regsGetByIdInfo._id))
    ...
    const handleSubmitAppDetails = async (e) => {
        ...
        dispatch(appDetailsDispatchUpdate(formData,regsGetByIdInfo._id))
    ...
    const handleSubmitExpense = async (e) => {
        ...
        dispatch(expenseDispatchUpdate(formData,regsGetByIdInfo._id))
    ...
    const handleSubmitLicence = (e) => {
        ...
        dispatch(licenseInfoDispatchUpdate(formData,regsGetByIdInfo._id))
    ...
    const handleSubmitInvoice = (e) => {
        ...
        dispatch(invoiceInfoDispatchUpdate(formData,regsGetByIdInfo._id))
    ...
    const handleSubmitCompanyInfo = async (e) => {
        ...
        // if(isDisabled === true){
            ...
            Swal.fire({
                icon: "success",
                title: "Your License/Registraion has been Updated.",
    ...
    useEffect(() => {
        setRn(expenseDetailsUpdateInfo?.regNo)
        if(namerateUpdateInfo?.regNo && expenseDetailsUpdateInfo?.regNo && expenseDetailsUpdateInfo?.docRemark && expenseDetailsUpdateInfo?.applicationRemark && expenseDetailsUpdateInfo?.challanNumber && licenseDetailsUpdateInfo?.licenseUpload && licenseDetailsUpdateInfo?.licenseUpload  && invoiceDetailsUpdateInfo?.invoiceType && companyInfoDetailsUpdateInfo?.branch){
            setIsDisabled(!isDisabled)
        }
    },[namerateUpdateInfo,expenseDetailsUpdateInfo,licenseDetailsUpdateInfo,invoiceDetailsUpdateInfo,companyInfoDetailsUpdateInfo])
                    ...
                    <div className="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                        <div className="row">
                            {/* <div className="col-md-4 col-lg-15 mb-2 mb-lg-3 mb-md-3" style={{ display:'none' }}>
                                <select className="form-select" aria-label="Default select example" id="company" name="company" value={company}  ref={myElementRefCompany} onChange={(e)=>{setCompany(e.target.value);getBbranch(e.target.value)}} required>
                            ...
                            <div className="col-md-4 col-lg-15 mb-2 mb-lg-3 mb-md-3" style={{  }}>
                                <button type="submit" className="w-100 btn btn-primary"  onClick={saveandapprove}>Approve</button>
                            </div>
                        ...
                        <div className="col-md-4 col-lg-15 mb-2 mb-lg-3 mb-md-3 p-2" >
                            <button type="submit" className="w-100 btn btn-danger"  /*onClick={reject}*/ onClick={showModal}>Reject</button>
                        </div>
                        <div className="col-md-4 col-lg-15 mb-2 mb-lg-3 mb-md-3 p-2" >
                            <button type="submit" className="w-100 btn btn-primary" >Share</button>
                        </div>
                    </div>
